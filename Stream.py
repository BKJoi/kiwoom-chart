...
# 💡 [속도 업그레이드] 모든 API 호출을 하나로 통합하고 max_pages(증분) 컨트롤 완벽 지원
...
def merge_api_data(old_data, new_data):
    ...
    return df_merged.drop_duplicates().to_dict('records')
...
if auto_refresh:
    ...
    st.sidebar.success("✅ 실시간 초고속 갱신 중...")
...
if auth_token and len(stock_number) == 6:
    ...
    # 💡 [핵심 패치 2] 최초 로딩 시에만 왕창 가져오고, 갱신 시에는 딱 2페이지만 가져와서 통신시간 극단적 단축
    if is_first_load:
        fetch_p = 500  
        ...
        if chart_raw:
            ...
            if pg_raw:
                ...
                if 'tm' in df_pg.columns and not df_pg.empty:
                    ...
                    # 💡 4층: PG 이상탐지 신기록 에디션 (어제 완성본 완벽 적용)
                    if avg_10d_pg_vol > 0:
            ...
            df_brk1 = process_broker_data(st.session_state['data_cache']['brk1'], lag_seconds, 'brk1')
            ...
            df_brk2 = process_broker_data(st.session_state['data_cache']['brk2'], lag_seconds, 'brk2')
            ...
            fig = make_subplots(
                ...
                subplot_titles=("가격", "거래량", "프로그램 수급", "🚨 프로그램 1분 폭발 (평균치 2이상 & 신기록)", f"{selected_broker_name1} 수급", f"{selected_broker_name2} 수급"),
